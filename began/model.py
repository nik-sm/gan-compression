...
class SimpleDiscriminator(nn.Module):
    def __init__(self, latent_dim, num_filters, image_size, num_dns, act='elu'):
        ...
        shape_after_conv = reduced_img_size**2 * ((num_dns+1) * num_filters) # N_pixels x N_channels

        convs = [DownBlock(num_filters, 1+i, act) for i in range(num_dns)]

        self.encoder = nn.Sequential(
                nn.Conv2d(3, num_filters, kernel_size=(3,3), padding=1), # TODO - bias=False?
                self.act,
                *convs,
                Flatten(),
                nn.Linear(shape_after_conv, latent_dim, bias=False))
...
class DownBlock(nn.Module):
    def __init__(self, num_filters, multiplier, act='elu'):
        ...
        self.conv_net = nn.Sequential(
            nn.Conv2d(num_filters * multiplier, num_filters*multiplier, kernel_size=(3,3), padding=1, bias=False),
            self.act,
            nn.Conv2d(num_filters * multiplier, num_filters*(multiplier+1), kernel_size=(3,3), padding=1, stride=2, bias=False),
            self.act)
...
class SimpleGenerator(nn.Module):
    def __init__(self, latent_dim, act='elu'):
        ...
        self.conv_net = nn.Sequential(
                UpBlock(up_scale=2, in_ch1=self.ch, in_ch2=self.ch, out_ch=self.ch, kern=(3,3), stride=(1,1), pad=(1,1), act=act, passthrough=True), # NOTE - want this one to NOT cat(conv(x), skip(x))
